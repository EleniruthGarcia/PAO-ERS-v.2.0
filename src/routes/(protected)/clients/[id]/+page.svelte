			...
			<Card.Header class="flex flex-row items-start bg-muted/50">
				<div class="grid gap-0.5">
					<Card.Title class="group flex items-center gap-2 text-lg">
						...
						>
							<Copy class="h-3 w-3" />
							<span class="sr-only">Copy data.client ID</span>
						</Button>
					</Card.Title>
					...
					<AlertDialog.Root>
						<AlertDialog.Trigger>
							<Button
								size="sm"
								variant="destructive"
								class="h-7 gap-1 bg-destructive text-sm">Delete</Button
							>
						</AlertDialog.Trigger>
						<AlertDialog.Content>
							<AlertDialog.Header>
								...
								<AlertDialog.Description>
									Are you aboslutely sure? The client will be archived and will not show up in
									Active Clients. If you want the client to be permanently deleted, please contact
									the administrator.
								</AlertDialog.Description>
							</AlertDialog.Header>
							<AlertDialog.Footer>
				...
				<div class="grid gap-3">
					...
					<ul class="grid gap-3">
						<li class="flex items-center justify-between">
							...
							<span class="truncate">{data.client.address}</span>
						</li>
						<li class="flex items-center justify-between">
					...
					<div class="grid gap-3">
						...
						<ul class="grid gap-3">
							<li class="flex items-center justify-between">
								...
								<span class="truncate">{data.client.spouseName}</span>
							</li>
							<li class="flex items-center justify-between">
								<span class="text-muted-foreground"> Address </span>
								<span class="truncate">{data.client.spouseAddress}</span>
							</li>
							<li class="flex items-center justify-between">
								<span class="text-muted-foreground"> Email </span>
					...
					<div class="grid gap-3">
						...
						<ul class="grid gap-3">
							<li class="flex items-center justify-between">
								...
								<span class="truncate">{data.client.detainedAt}</span>
							</li>
							<li class="flex items-center justify-between">
								<span class="text-muted-foreground"> Detained Since </span>
								<span class="truncate">{data.client.detainedSince}</span>
							</li>
						</ul>
					</div>
				{/if}
