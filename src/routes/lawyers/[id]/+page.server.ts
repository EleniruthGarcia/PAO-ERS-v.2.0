...
export const actions = {
    ...
    edit: async ({ request, params }) => {
        ...
        if (typeof username !== 'string' || typeof title !== 'string' || typeof firstName !== 'string' || typeof middleName !== 'string' || typeof lastName !== 'string' || typeof Number(age) !== 'number' || typeof sex !== 'string' || typeof address !== 'string') {
        ...
        const lawyer = await prisma.lawyer.update({
            where: { id: Number(params.id) },
            include: { user: true },
            data: { user: { update: { data: { username } } }, title, firstName, middleName, lastName, nameSuffix, age: Number(age), sex, address, email, contactNumber }
        });
