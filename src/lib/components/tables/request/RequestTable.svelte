...
<Tabs.Root value="all">
	...
	<Tabs.Content value="all">
		<Card.Root>
			<Card.Header class="px-7">
				...
				<Card.Description>All requests added to the system are shown here.</Card.Description>
			</Card.Header>
			<Card.Content>
				{#await $page.data.requests}
					<Loading />
				{:then requests}
					{#if requests.filter((c) => c.status?.at(-1)?.type !== 'Archived').length > 0}
						<Table data={requests.filter((c) => c.status?.at(-1)?.type !== 'Archived')} />
					{:else}
						<div
						...
						>
							<div class="flex flex-col items-center gap-1 text-center">
								...
								<p class="text-sm text-muted-foreground">
									You can start using the system as soon as you add a new request.
								</p>
	...
	<Tabs.Content value="recents">
		<Card.Root>
			<Card.Header class="px-7">
				...
				<Card.Description>New requests added to the system are shown here.</Card.Description>
			</Card.Header>
			<Card.Content>
				{#await $page.data.requests}
					<Loading />
				{:then requests}
					{#if requests.filter((c) => c.status?.at(-1)?.type === 'New').length > 0}
						<Table data={requests.filter((c) => c.status?.at(-1)?.type === 'New')} />
					{:else}
						<div
							class="flex h-full flex-1 items-center justify-center rounded-lg border border-dashed border-muted-foreground/50 p-6 shadow-sm"
						>
							<div class="flex flex-col items-center gap-1 text-center">
								...
								<p class="text-sm text-muted-foreground">
									You can start using the system as soon as you add a new requests.
								</p>
								<Button class="mt-4" href="/requests/add">Add Request</Button>
							</div>
						</div>
					...
					{:else}
						...
						>
							<div class="flex flex-col items-center gap-1 text-center">
								...
								<p class="text-sm text-muted-foreground">You may view your archived requests here.</p>
								<Button class="mt-4" href="/requests/add">Add Case</Button>
							</div>
						</div>
					{/if}
				{/await}
	...
	<Tabs.Content value="terminated">
		<Card.Root>
			<Card.Header class="px-7">
				<Card.Title>Terminated Requests</Card.Title>
				<Card.Description>All terminated requests are shown here.</Card.Description>
			</Card.Header>
			<Card.Content>
				{#await $page.data.requests}
					<Loading />
				{:then requests}
					{#if requests.filter((c) => c.status?.at(-1)?.type === 'Terminated').length > 0}
						<Table data={requests.filter((c) => c.status?.at(-1)?.type === 'Terminated')} />
					{:else}
						<div
							class="flex h-full flex-1 items-center justify-center rounded-lg border border-dashed border-muted-foreground/50 p-6 shadow-sm"
						>
							<div class="flex flex-col items-center gap-1 text-center">
								<h3 class="text-2xl font-bold tracking-tight">You have no terminated requests.</h3>
								<p class="text-sm text-muted-foreground">
									You may view your terminated requests here.
								</p>
								<Button class="mt-4" href="/requests/add">Add Request</Button>
							</div>
						</div>
