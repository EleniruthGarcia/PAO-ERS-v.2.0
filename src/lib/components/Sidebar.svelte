...
<aside class="overflow-y-hidden flex flex-col items-center justify-between h-screen bg-verdict py-6 w-16 sm:w-96 sm:items-start sm:p-10">
    <span class="flex flex-col items-center gap-16 mt-4 sm:items-start">
        ...
        <nav class="flex flex-col items-center text-oath gap-2 sm:items-start">
            ...
            <a href="/" class={$page.route.id === "/" ? "flex font-bold hover:bg-equity p-4 py-2 rounded-full gap-4" : "flex hover:bg-equity p-4 py-2 rounded-full gap-4"}>
                <Dashboard />
                <h4 class="hidden md:block">Dashboard</h4>
            </a>
            <a href="/cases" class={$page.route.id === "/cases" ? "flex font-bold hover:bg-equity p-4 py-2 rounded-full gap-4" : "flex hover:bg-equity p-4 py-2 rounded-full gap-4"}>
                <Cases />
                <h4 class="hidden md:block">Cases</h4>
            </a>
            {#if $page.data.user.role === 'lawyer'}
                <a href="/clients" class={$page.route.id === "/clients" ? "flex font-bold hover:bg-equity p-4 py-2 rounded-full gap-4" : "flex hover:bg-equity p-4 py-2 rounded-full gap-4"}>
                    <Clients />
                    <h4 class="hidden md:block">Clients</h4>
                </a>
            {/if}
            {#if $page.data.user.role === 'admin'}
                <a href="/lawyers" class={$page.route.id === "/lawyers" ? "flex font-bold hover:bg-equity p-4 py-2 rounded-full gap-4" : "flex hover:bg-equity p-4 py-2 rounded-full gap-4"}>
                    <Lawyers />
                    <h4 class="hidden md:block">Lawyers</h4>
                </a>
            {/if}
            <a href="/reports" class={$page.route.id === "/reports" ? "flex font-bold hover:bg-equity p-4 py-2 rounded-full gap-4" : "flex hover:bg-equity p-4 py-2 rounded-full gap-4"}>
                <Reports />
                <h4 class="hidden md:block">Reports</h4>
            </a>
            {#if $page.data.user.role === 'lawyer'}
                <a href="/recents" class={$page.route.id === "/recents" ? "flex font-bold hover:bg-equity p-4 py-2 rounded-full gap-4" : "flex hover:bg-equity p-4 py-2 rounded-full gap-4"}>
                    <Recents />
                    <h4 class="hidden md:block">Recent Services</h4>
                </a>
            {/if}
            {#if $page.data.user.role === 'admin'}
                <a href="/settings" class={$page.route.id === "/settings" ? "flex font-bold hover:bg-equity p-4 py-2 rounded-full gap-4" : "flex hover:bg-equity p-4 py-2 rounded-full gap-4"}>
                    <Settings />
    ...
    <div class="group flex flex-col items-center justify-center w-full text-oath pl-4 gap-4 sm:items-start">
